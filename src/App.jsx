...
function App() {
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-neon-green selection:text-black">
      ...
      {/* NAVBAR (No modificado por solicitud) */}
      ...
      <section className="max-w-6xl mx-auto px-6 py-20 space-y-24">
        {/* PROTOCOLO DE SEGURIDAD (MÁS INFO) */}
        ...
        <div className="relative h-[300px] md:h-[500px] w-full overflow-hidden rounded-3xl border border-white/10 group">
          {slides.map((slide, index) => (
            <div key={index} className={`absolute inset-0 transition-opacity duration-1000 ${index === currentSlide ? "opacity-100" : "opacity-0"}`}>
              ...
              <div className="absolute bottom-10 left-10 z-20 text-left text-left">
        ...
        {/* MISIÓN Y VISIÓN (ADAPTADO AL PROYECTO) */}
        ...
        {/* FILOSOFÍA DEL GUERRERO (RESUMIDA E INFORMATIVA) */}
        <div className="pt-20 border-t border-neon-green/10">
          <div className="bg-gradient-to-br from-gray-900 to-black border border-white/5 rounded-3xl p-10 shadow-2xl relative overflow-hidden text-left">
            <div className="absolute top-0 right-0 p-8 opacity-5"><Award size={200} className="text-white" /></div>
            <h3 className="text-3xl font-black italic uppercase mb-8 flex items-center gap-3 text-left"><span className="w-2 h-8 bg-neon-green"></span> Filosofía del <span className="text-neon-green">Guerrero Gratuito</span></h3>
            <div className="grid md:grid-cols-2 gap-12 relative z-10">
              <div className="space-y-6 text-gray-400 leading-relaxed text-sm font-medium uppercase tracking-tight text-left">
                <p>Nuestra plataforma nace bajo el concepto de "Dojo Abierto". Creemos firmemente que el conocimiento sobre nutrición y entrenamiento no debe estar bajo un muro de pago.</p>
                <p className="text-white/70">Shonen Workout es un proyecto sin fines de lucro que utiliza la motivación de la cultura pop para inspirar disciplina, ofreciendo herramientas de gestión de datos que normalmente costarían una suscripción premium, de forma totalmente abierta.</p>
              </div>
              <div className="bg-black/50 p-8 rounded-2xl border border-yellow-400/20 text-left">
                <h4 className="text-yellow-400 font-bold uppercase tracking-widest text-sm flex items-center gap-2 mb-4 font-black"><Coffee size={20} /> Nodo de Sostenibilidad</h4>
                <p className="text-[10px] text-gray-500 leading-loose uppercase font-bold tracking-widest mb-6">El mantenimiento de servidores y base de datos Supabase depende de la comunidad.</p>
                <Link to="/donaciones" className="w-full block text-center bg-yellow-400/10 text-yellow-400 border border-yellow-400/30 px-6 py-3 rounded-xl font-black uppercase text-xs hover:bg-yellow-400 hover:text-black transition-all">Mantener Proyecto Online</Link>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* FOOTER (ACTUALIZADO CON INFRAESTRUCTURA FINAL) */}
